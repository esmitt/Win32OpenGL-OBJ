...
{
	// The is a list of triangles (e.g. GL_TRIANGLES) stored in m_indices.
	...
	//
	// The transformation matrix trans, transforms the position of all vertices
	// so that the rendered cube is transformed by the matrix
	glBegin(GL_TRIANGLES);
	for (int i = 0; i < 36; i++)
	{
		Vector4D pos = *trans * m_vertices[m_indices[i]].position;
